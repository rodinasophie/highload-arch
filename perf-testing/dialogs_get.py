...
def create_users(filename, users):
    ...
    for row in df.itertuples():
        r = send_request(method='POST', url='http://localhost:8070/user/register',
        ...
        if r.status_code != 200:
            ...
            r2 = send_request(method='POST', url='http://localhost:8070/login',
                           json = {
                              "id": r.json()['user_id'],
                              "password": DEFAULT_PASSWORD
                           })
...
def init_dialogs(n):
    for i in range(n):
        ...
        send_request(method='POST', url = f"http://localhost:8070/dialog/" +  user1 + f"/send", json = {"text": message[:400]}, token = users[user2])
        send_request(method='POST', url = f"http://localhost:8070/dialog/" +  user2 + f"/send", json = {"text": message[:400]}, token = users[user1])
